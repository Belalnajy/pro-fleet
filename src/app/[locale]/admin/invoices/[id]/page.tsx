...
export default function InvoiceDetailsPage({ params: pageParams }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  const handleSendEmail = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/invoices/${id}/send-email`, {
        method: 'POST'
      })
                  ...
                  onPaymentAdded={(updatedInvoice) => {
                    ...
                    if (updatedInvoice) {
                      setInvoice(prev => prev ? {
                        ...
                        remainingAmount: updatedInvoice.remainingAmount !== undefined ? updatedInvoice.remainingAmount : (prev.total - (updatedInvoice.amountPaid || 0)),
