...
export async function GET(
  ...
  try {
    ...
    // Generate simple English HTML content for PDF
    const htmlContent = `
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body>
        ...
        <div class="invoice-info">
            ...
            <p><strong>Issue Date:</strong> ${new Date(invoice.createdAt).toLocaleDateString("en-US")}</p>
        ...
        <div class="customer-info">
            ...
            <p><strong>${invoice.trip?.customer?.name || "Customer Not Specified"}</strong></p>
        ...
        <div class="trip-info">
            ...
            <p><strong>Route:</strong> ${invoice.trip.fromCity?.name} → ${invoice.trip.toCity?.name}</p>
            <p><strong>Driver:</strong> ${invoice.trip.driver?.user?.name || "Not Specified"}</p>
            <p><strong>Vehicle:</strong> ${invoice.trip.vehicle?.vehicleType?.nameAr || "Not Specified"}</p>
        </div>
        `
            : ""
        }
        ...
        <div class="totals">
            <div class="total-row">
                <strong>Subtotal: ${invoice.subtotal.toFixed(2)} ${invoice.currency}</strong>
            </div>
            ${
              invoice.customsFee > 0
                ? `
            <div class="total-row">
                <strong>Customs Fee: ${invoice.customsFee.toFixed(2)} ${invoice.currency}</strong>
            </div>
            `
                : ""
            }
            <div class="total-row">
                <strong>VAT (${(invoice.taxRate * 100).toFixed(0)}%): ${invoice.taxAmount.toFixed(2)} ${invoice.currency}</strong>
            </div>
            <div class="final-total">
                <strong>Total Amount: ${invoice.total.toFixed(2)} ${invoice.currency}</strong>
            </div>
        </div>
