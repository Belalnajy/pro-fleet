...
export async function POST(
  ...
  try {
    ...
    if (amount > remaining) {
      return NextResponse.json({ 
        error: `Payment amount cannot exceed remaining balance of ${remaining.toFixed(2)}` 
      }, { status: 400 })
    }
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      if (invoice.paymentStatus === 'INSTALLMENT' && invoice.installmentAmount) {
        ...
        if (newPaymentStatus !== 'PAID' && invoice.installmentCount && newInstallmentsPaid < invoice.installmentCount) {
    ...
    const updatedInvoiceData = {
      ...
      nextInstallmentDate: result.updatedInvoice.nextInstallmentDate?.toISOString(),
