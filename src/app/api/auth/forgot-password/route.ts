...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate reset URL
    const resetUrl = `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/auth/reset-password?token=${result.token}`

    // Send email
    if (result.success) {
      const emailTemplate = getPasswordResetEmailTemplate(resetUrl, user?.name || 'المستخدم')
      try {
        await sendEmail({
          to: email,
