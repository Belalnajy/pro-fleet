...
async function extractCustomsBrokerFromNotes(
  notes: string | null
): Promise<string | null> {
  ...
  try {
    ...
    console.log(
      "🔍 [INLINE] Available brokers:",
      customsBrokers.map((b) => b.user.name)
    );

    const foundBroker = customsBrokers.find(
      (broker) => broker.user.name.toLowerCase() === brokerName.toLowerCase()
    );

    if (foundBroker) {
      console.log(
        "🔍 [INLINE] Found matching broker:",
        foundBroker.user.name,
        "ID:",
        foundBroker.id
      );
...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedInvoices = invoices.map((invoice) => ({
      ...
      customerId: invoice.trip?.customer?.id || invoice.customerId || '',
      customerName: invoice.customerName || invoice.trip?.customer?.name || 'عميل غير محدد',
  ...
  try {
    ...
    if (tripId) {
      ...
      if (trip?.customsBrokerId) {
        console.log(
          "✅ [INVOICE CREATION] Customs broker found in trip:",
          trip.customsBrokerId
        );
      } else {
    ...
    if (customsBrokerId) {
      console.log(
        "✅ [INVOICE CREATION] Invoice will be linked to customs broker:",
        customsBrokerId
      );
    } else {
      console.log(
        "⚠️ [INVOICE CREATION] Invoice will NOT have customs broker link"
      );
    }
    ...
    if (customsBrokerId) {
      ...
      try {
        console.log(
          "🔄 [AUTO CLEARANCE] Creating automatic clearance for invoice:",
          invoice.invoiceNumber
        );

        // Generate clearance number
        const clearanceCount = await db.customsClearance.count();
        const clearanceNumber = `CL-${String(clearanceCount + 1).padStart(
          6,
          "0"
        )}`;
        ...
        const clearance = await db.customsClearance.create({
          data: {
            clearanceNumber,
            invoiceId: invoice.id,
            customsBrokerId: customsBrokerId!, // Use the variable we already have
        ...
        console.error(
          "❌ [AUTO CLEARANCE] Failed to create automatic clearance:",
          clearanceError
        );
      ...
      console.log(
        "❌ [INVOICE CREATION] WARNING: Invoice is NOT linked to any customs broker"
      );
    }
