    ...
    <div className="space-y-3">
      ...
      {value ? (
        <div className="relative">
          <div className="flex items-start gap-3 p-4 border rounded-lg bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
            <div className={`w-3 h-3 rounded-full mt-1 flex-shrink-0 ${type === 'origin' ? 'bg-green-500' : 'bg-red-500'}`}></div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-2">
                <MapPin className={`w-4 h-4 ${type === 'origin' ? 'text-green-600' : 'text-red-600'}`} />
        ...
        <Button
          variant="outline"
          onClick={() => setShowPicker(true)}
          className="w-full h-16 border-dashed border-2 hover:border-solid hover:border-blue-300 hover:bg-blue-50 transition-all duration-200 group"
        >
