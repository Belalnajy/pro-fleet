...
export const CITY_COORDINATES: Record<string, { lat: number; lng: number }> = {
  ...
  Tabuk: { lat: 28.3998, lng: 36.57 },
  Buraidah: { lat: 26.326, lng: 43.975 },
  "Khamis Mushait": { lat: 18.3, lng: 42.73 },
...
export function getCityCoordinates(cityName: string): {
  ...
  const coords =
    CITY_COORDINATES[cityName] || CITY_COORDINATES[cityName.trim()];
  if (coords) {
    return coords;
  }
...
// Check if coordinates match a known city (with tolerance for GPS precision)
...
// Get display name for location (city name if known, custom location if not)
export function getLocationDisplayName(
  ...
  const cityName = findCityByCoordinates(lat, lng);
  ...
  return {
    name: fallbackName || `موقع مخصص: ${lat.toFixed(6)}, ${lng.toFixed(6)}`,
    isKnownCity: false
  };
}
